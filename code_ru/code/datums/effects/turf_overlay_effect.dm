/datum/effects/turf_overlay_effect //should make this generic and the water_overlay a seperate thing
	...
	icon_path = 'code_ru/icons/effects/water_effects.dmi'
...
/obj/effect/turf_overlay_effect/proc/set_up_icon(turf/open/input_openturf, mob/living/carbon/human/input_human, pixel_y_offset = 0)
	if(input_human.body_position == LYING_DOWN)
		...
		switch(input_human.transform.b)
			if(90)
				matrix.Turn(270)
			if(270) //but I noticed these values were unique between the laying directions :0)
				matrix.Turn(90)
		pixel_y_offset = -12
	...
	var/icon/subtraction_texture = icon('code_ru/icons/effects/water_effects.dmi',"culling_mask")
